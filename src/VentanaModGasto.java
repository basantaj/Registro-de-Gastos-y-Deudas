...
public class VentanaModGasto extends JFrame {
	...
	public void mostrar() {
		setTitle("Modificacion de Gastos");
		setResizable(false);
		...
		final JButton borraBtn = new JButton("Modificar");
		borraBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ModGasto mo  = new ModGasto();
				BuscarGasto bu = new BuscarGasto();
				try {
					if (bu.buscar(compraTxt.getText())){
					mo.modificar(compraTxt.getText(), fechaTxt.getText(), costoTxt.getText(), vendedorTxt.getText(), tarjetaCombo.getSelectedItem().toString(), importadoCombo.getSelectedItem().toString());
					}
		...
		JLabel lblIngresoDeGasto = new JLabel("MODIFICADO DE GASTO");
		lblIngresoDeGasto.setFont(new Font("Tahoma", Font.PLAIN, 24));
		lblIngresoDeGasto.setBounds(97, 11, 302, 44);
